...
import se.lth.cs.docforia.data.JsonCoreReader;
import se.lth.cs.docforia.data.JsonCoreWriter;
...
public class MemoryJsonLevel0Codec extends MemoryJsonCodec {
    ...
    private static class Writer {
        ...
        private JsonCoreWriter propwriter;

        public Writer(JsonGenerator writer) {
            this.writer = writer;
            this.propwriter = new JsonCoreWriter(writer);
        }
        ...
        public int writeNodes(int idcounter, MemoryNodeCollection collection, Reference2IntOpenHashMap<NodeRef> refs) {
            try {
                int numPureNodes = 0;
                ...
                for (NodeRef nodeRef : collection) {
                    MemoryNode nodeStore = (MemoryNode) nodeRef.get();
                    writeProperties(nodeStore.properties);
                }
                writer.writeEndArray();
        ...
        public int writeNodeLayer(int idcounter, ArrayList<MemoryNodeCollection> layerGroup, Reference2IntOpenHashMap<NodeRef> refs) {
            try {
                writer.writeStartObject();
        ...
        public void writeEdges(MemoryEdgeCollection collection, Reference2IntOpenHashMap<NodeRef> refs) {
            try {
                writer.writeStartObject();
        ...
        public void writeEdgeLayer(ArrayList<MemoryEdgeCollection> layerGroup, Reference2IntOpenHashMap<NodeRef> refs) {
            try {
                writer.writeStartObject();
                writer.writeObjectField("layer", layerGroup.get(0).getKey().layer);
                writer.writeArrayFieldStart("variants");
        ...
        public void writeEdgeLayers(MemoryDocument doc, Reference2IntOpenHashMap<NodeRef> refs) {
            String lastLayer = "";
        ...
        public int writeNodeLayers(MemoryDocument doc, Reference2IntOpenHashMap<NodeRef> refs) {
            String lastLayer = "";
    ...
    private static class Reader {
        ...
        private JsonCoreReader reader;

        public Reader() {
            this.reader = new JsonCoreReader();
        }

        public void readNodes(JsonNode node, MemoryNodeCollection collection, Int2ReferenceOpenHashMap<NodeRef> nodeRefs) {
    ...
    public void encode(MemoryDocument doc, JsonGenerator jsonWriter) {
        try {
            Writer writer = new Writer(jsonWriter);
            ...
            for (String s : doc.store.properties.keySet()) {
                ...
                if(ref instanceof CoreRef) {
                    ((CoreRef) ref).write(writer.propwriter);
                } else {
            ...
            writer.writeNodeLayers(doc, refs);
            jsonWriter.writeEndArray();

            //Write edges
            jsonWriter.writeArrayFieldStart("edges");
            writer.writeEdgeLayers(doc, refs);
            jsonWriter.writeEndArray();
    ...
    public MemoryDocument decode(JsonNode jsonNode) {
        ...
        store.properties = reader.readProperties(properties);
        ...
        reader.readNodeLayers(nodes, store, nodeRefs);

        JsonNode edges = docData.path("edges");
        reader.readEdgeLayers(edges, store, nodeRefs);
